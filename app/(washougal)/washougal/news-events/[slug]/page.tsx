...
export async function generateMetadata(
  ...
  if (!post) return {}
  return {
    title: `${post.title} | Arcadia Senior Living Washougal`,
    description: post.excerpt,
    alternates: { canonical: `https://arcadiaretirement.com/washougal/news-events/${slug}` },
    openGraph: { images: [{ url: post.image }] },
  }
}

export default async function WashougalPostPage(
  { params }: { params: Promise<{ slug: string }> }
) {
  const { slug } = await params
  const post = getPostBySlug(slug)
      ...
      <section className="relative min-h-[280px] md:min-h-[340px] flex items-center bg-[#2d5a5c] overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-[900px] mx-auto px-6 lg:px-8 py-14">
          <nav className="flex items-center gap-2 text-white/50 text-xs uppercase tracking-widest mb-5">
            <Link href="/washougal" className="hover:text-white transition-colors">Washougal</Link>
            <span>/</span>
            <Link href="/washougal/news-events" className="hover:text-white transition-colors">News &amp; Events</Link>
          </nav>
            ...
            <aside className="space-y-6 lg:sticky lg:top-28">
              ...
              <div className="bg-[#2d5a5c] p-6">
                ...
                <p className="text-white/65 text-sm leading-relaxed mb-5">
                  Schedule a personal tour and see why families trust Arcadia Senior Living Lookout Ridge.
                </p>
              ...
              {related.length > 0 && (
                <div className="bg-[#f9f9f9] p-6">
                  ...
                  <ul className="space-y-4">
                    {related.map(r => (
                      <li key={r.slug}>
                        <Link href={`/washougal/news-events/${r.slug}`} className="flex gap-3 group">
                  ...
                  <Link href="/washougal/news-events"
