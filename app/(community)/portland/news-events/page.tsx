...
export const metadata: Metadata = {
  title: "What's Happening at Arcadia Portland | News & Events",
  description: 'Stay up to date with the latest news, events, and happenings at Arcadia Senior Living Portland — from seasonal celebrations to community open houses.',
  alternates: { canonical: 'https://arcadiaretirement.com/portland/news-events' },
  openGraph: {
    title: "What's Happening at Arcadia Portland | News & Events",
    description: 'Latest news, events, and happenings at Arcadia Senior Living Portland.',
    ...
    <main>
      ...
      <section className="relative min-h-[280px] md:min-h-[340px] flex items-center bg-[#2d5a5c] overflow-hidden">
        <div className="absolute inset-0 opacity-10"
          style={{ backgroundImage: `url('https://arcadiaretirement.com/portland/wp-content/uploads/sites/4/2025/11/10.17-happy-hour-1.jpg')`, backgroundSize: 'cover', backgroundPosition: 'center' }} />
        <div className="absolute inset-0 bg-[#2d5a5c]/80" />
        <div className="relative z-10 w-full text-center px-6 py-16">
          <p className="text-white/65 text-sm uppercase tracking-[0.4em] mb-3">Arcadia Senior Living Portland</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {posts.map(post => (
              <article key={post.slug} className="bg-white shadow-sm overflow-hidden flex flex-col group hover:shadow-md transition-shadow duration-300">
                ...
                {/* Image */}
                <Link href={`/portland/${post.slug}`} className="block overflow-hidden">
                  <img
                    src={post.image}
                    alt={post.imageAlt}
                ...
                <div className="flex flex-col flex-1 p-6 gap-4">
                  ...
                  <h2 className="font-serif text-lg font-light text-[#1a1a1a] leading-snug line-clamp-3">
                    <Link href={`/portland/${post.slug}`} className="hover:text-[#2d5a5c] transition-colors">
                      {post.title}
                    </Link>
                  </h2>
